...
from .cycle_sde import (Sampler, get_img_latent, get_text_embed,
                           load_model, set_seed)
...
def forward(img, scheduler, sampler, steps, drag_t, lora_scale_min, text_embeddings):
    ...
    index = 1
    for t in scheduler.timesteps[t_begin:].flip(dims=[0]):
...
def backward(scheduler, sampler, mask, steps, drag_t, latents, noises, hook_latents, lora_scales, cfg_scales, text_embeddings):
    drag_t = int(drag_t * steps)
    t_begin = steps - drag_t

    hook_latent = hook_latents.pop()
    # latents = torch.where(mask > 128, latents, hook_latent)
    for t in scheduler.timesteps[t_begin - 1:-1]:
...
def copy_and_paste(latents, source_new, target_new, r, max_height, max_width, img_scale, noise_scale, device):
    ...
    for source_, target_ in zip(source_new, target_new):
        ...
        latents[:, :, source_[1] - r_y_lower: source_[1] + r_y_upper, source_[0] - r_x_lower: source_[0] + r_x_upper] = \
            img_scale * source_feature + noise_scale * torch.randn(latents.shape[0], 4, r_y_lower + r_y_upper, r_x_lower + r_x_upper, device=device)

        latents[:, :, target_[1] - r_y_lower: target_[1] + r_y_upper, target_[0] - r_x_lower: target_[0] + r_x_upper] = source_feature * 1.1
    return latents


def drag(img_path, lora_unet, source, target, mask_path=None, prompt="", drag_t=0.6, steps=50, step_size=2, img_scale=0.3, adapt_r=5, seed=1234):
    set_seed(seed)
    ...
    if mask_path is not None:
        ...
        mask = torch.ones((img.shape[2], img.shape[3]))*255
    ...
    for i in range(int(drag_num)):
        ...
        if use_lora:
            with torch.autocast(device):
                latents, noises, hook_latents, lora_scales, cfg_scales = forward(img, scheduler, sampler, steps, drag_t, lora_scale_min, text_embeddings)
                latents = copy_and_paste(latents, source_new, target_new, adapt_r, img.shape[2] - 1, img.shape[3] - 1, img_scale, noise_scale, device)
                img = backward(scheduler, sampler, mask, steps, drag_t, latents, noises, hook_latents, lora_scales, cfg_scales, text_embeddings)
        else:
            latents, noises, hook_latents, lora_scales, cfg_scales = forward(img, scheduler, sampler, steps, drag_t, lora_scale_min, text_embeddings)
            latents = copy_and_paste(latents, source_new, target_new, adapt_r, img.shape[2] - 1, img.shape[3] - 1, img_scale, noise_scale, device)
            img = backward(scheduler, sampler, mask, steps, drag_t, latents, noises, hook_latents, lora_scales, cfg_scales, text_embeddings)
